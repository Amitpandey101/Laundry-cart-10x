...
const createorder= async (req,res)=>{
    try{
        ...
        UserData.find({email:email}).then((userdata)=>{
         if(userdata.length){
             OrderData.create({date:req.body.date,totalPrice:req.body.totalPrice,totalItems:req.body.totalItems,status:req.body.status,products:req.body.products,user:userdata[0].email}).then((order)=>{
